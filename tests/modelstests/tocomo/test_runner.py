...
tocomo_runner = importlib.import_module("internshipwork.models.tocomo.runner")


def test_build_storage_row_handles_zero_total_flow_with_terminal_merge():
    results = [
        {
            "source_type": "merge",
            "source_name": "Merge A",
            "stream_phase": "gas",
            "density_kg_per_m3": 2.5,
    ...
    storage = tocomo_runner._build_storage_row(results, merge_definitions, storage_name="Tank")

    assert storage is not None
    assert storage["source_type"] == "storage"
    assert storage["source_name"] == "Tank"
    assert storage["total_massflow"] is None
    assert storage["tocomo_input"] == {"SO2": 4.0}
...
def test_build_storage_row_mixes_terminal_rows_weighted():
    ...
    storage = tocomo_runner._build_storage_row(results, merge_definitions)
